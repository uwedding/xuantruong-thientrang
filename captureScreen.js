...
$(document).on("click", ".cap_downloadBtn", function () {
  ...
  html2canvas(document.querySelector(".cap_screen"), { useCORS: true })
    .then((canvas) => {
      canvas.toBlob(function (blob) {
        if (isSafari) {
          // Sá»­ dá»¥ng FileReader Ä‘á»ƒ chuyá»ƒn Ä‘á»•i Blob thÃ nh Data URL
          var reader = new FileReader();
          reader.onload = function (e) {
            var newWindow = window.open("", "_blank");
            var imgElement = newWindow.document.createElement("img");
            imgElement.src = e.target.result; // URL tá»« FileReader
            ...
            downloadButton.textContent = "Táº£i vá»";
            downloadButton.style.cssText =
              "display: inline-block; margin-top: 20px; padding: 10px 20px; background-color: #4CAF50; color: white; text-decoration: none; border-radius: 5px;";
    ...
    .catch((error) => {
      console.error("Lá»—i khi chá»¥p áº£nh mÃ n hÃ¬nh:", error);
    });
